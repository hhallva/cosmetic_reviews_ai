...
{
  "executive_summary": "<Связный аналитический текст. Какие системные проблемы/преимущества выявлены, что следует делать компании на стратегическом уровне. Сформулировать практические рекомендации для маркетолога компании на основе выявленных потребительских настроений, проблемных зон и положительных характеристик продукции>",
  "pain_points": [
    {"category": "<название категории боли>", "frequency": "<высокая|средняя|низкая> исходя из количества упоминания", "examples": ["<пример 1>", "<пример 2>"]}
  ],
  "strengths": [
    {"category": "<название сильной стороны>", "frequency": "<высокая|средняя|низкая> исходя из количества упоминания", "examples": ["<пример 1>"]}
  ],
  "recommendations": [
...
def analyze_with_Yandex(data: Dict[str, Any], reviews: List[Any]) -> Optional[Dict[str, Any]]:
    ...
    reviews_text = _format_reviews_for_llm(reviews, max_reviews=250)
